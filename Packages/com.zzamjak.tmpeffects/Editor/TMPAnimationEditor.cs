...
namespace CAT.UI
{
    ...
    public class TMPAnimationEditor : Editor
    {
        ...
        private bool _showImpactSection = false;
        ...
        private void DrawPlaybackSection()
        {
            ...
            if (_showPlaybackSection)
            {
                ...
                {
                    // 에디터 모드 — 런타임에서 Play() / Restart() 직접 호출과 동일하게 검증 (AdvanceAnimation은 에디터 업데이트로 구동)
                    EditorGUILayout.BeginHorizontal();
                    ...
                    EditorGUILayout.LabelField("Status", _isEditorPlaying ? "Editor Preview (런타임 API)" : "Stopped", EditorStyles.miniLabel);
                }

                // Trigger Impact 버튼 (Impact 활성 시 Play 모드 또는 에디터 프리뷰 중에만 작동)
                if (_target.EnableImpact)
                {
                    EditorGUILayout.Space(3);
                    bool canTrigger = (Application.isPlaying && _target.IsPlaying) || _isEditorPlaying;
                    GUI.enabled = canTrigger && !_target.IsImpactPlaying;
                    GUI.backgroundColor = new Color(1f, 0.85f, 0.4f);
                    if (GUILayout.Button("Trigger Impact", GUILayout.Height(25)))
                    {
                        _target.TriggerImpact();
                    }
                    GUI.backgroundColor = Color.white;
                    GUI.enabled = true;

                    string impactStatus = _target.IsImpactPlaying ? "Impact Playing" : (canTrigger ? "Ready" : "재생 중에만 가능");
                    EditorGUILayout.LabelField("Impact", impactStatus, EditorStyles.miniLabel);
                }
        ...
        private void DrawImpactSection()
        {
            var enableImpact = serializedObject.FindProperty("_enableImpact");

            // Foldout + Enable 토글 조합 헤더 (Appear 패턴 답습)
            ...
            _showImpactSection = EditorGUI.Foldout(foldoutRect, _showImpactSection, GUIContent.none, true);
            enableImpact.boolValue = EditorGUI.ToggleLeft(toggleRect, "Impact Animation", enableImpact.boolValue, EditorStyles.boldLabel);

            if (_showImpactSection && enableImpact.boolValue)
            {
                EditorGUILayout.BeginVertical(EditorStyles.helpBox);
                EditorGUI.indentLevel++;

                EditorGUILayout.HelpBox(
                    "런타임 이벤트 트리거 기반 단발성 연출. TriggerImpact() 호출 시 재생되며, " +
                    "Appear/Loop/Disappear 위에 오프셋으로 겹쳐져 합성됩니다.",
                    MessageType.None
                );

                DrawVector2Property("_impactScale", "Peak Scale");
                DrawVector2Property("_impactPosition", "Peak Position");
                DrawRotationZProperty("_impactRotation", "Peak Rotation");

                EditorGUILayout.Space(3);

                EditorGUILayout.PropertyField(serializedObject.FindProperty("_impactDuration"), new GUIContent("Duration"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("_impactCurve"), new GUIContent("Curve (0→1)"));
                EditorGUILayout.PropertyField(serializedObject.FindProperty("_impactCharacterDelay"), new GUIContent("Character Delay"));
        ...
        private void DrawTimingSection()
        {
            ...
            if (_showTimingSection)
            {
                ...
                DrawPivotProperty("_pivot", "Pivot");
        ...
        /// <summary>
        /// Pivot 필드를 프리셋 버튼과 함께 표시 (Bottom/Center/Top).
        /// 정규화 좌표 (X: 0=좌변, 0.5=중앙, 1=우변 / Y: 0=베이스라인, 1=ascender)
        /// </summary>
        private void DrawPivotProperty(string propertyName, string label)
        {
            var prop = serializedObject.FindProperty(propertyName);
            var vec = prop.vector2Value;

            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField(new GUIContent(label, "스케일/회전 피벗 (정규화). X: 좌변~우변, Y: 베이스라인~상단"),
                GUILayout.Width(EditorGUIUtility.labelWidth - 15));
            ...
            if (GUILayout.Button("Baseline", EditorStyles.miniButton)) vec = new Vector2(0.5f, 0f);
            if (GUILayout.Button("Center", EditorStyles.miniButton)) vec = new Vector2(0.5f, 0.5f);
            if (GUILayout.Button("Top", EditorStyles.miniButton)) vec = new Vector2(0.5f, 1f);
            EditorGUILayout.EndHorizontal();
        ...
        private void DrawAppearSection()
        {
            ...
            if (_showAppearSection && enableAppear.boolValue)
            {
                ...
                // Ease Curve
                EditorGUILayout.PropertyField(serializedObject.FindProperty("_appearCurve"), new GUIContent("Curve"));
        ...
        private void DrawLoopSection()
        {
            ...
            if (_showLoopSection && enableLoop.boolValue)
            {
                ...
                // Ease Curve
                EditorGUILayout.PropertyField(serializedObject.FindProperty("_loopCurve"), new GUIContent("Curve"));
        ...
        private void DrawDisappearSection()
        {
            ...
            else if (_showDisappearSection && enableDisappear.boolValue)
            {
                ...
                // Ease Curve
                EditorGUILayout.PropertyField(serializedObject.FindProperty("_disappearCurve"), new GUIContent("Curve"));
        ...
        private bool HasValuesChanged()
        {
            ...
            return _target.CharacterDelay != _selectedPreset.CharacterDelay ||
                   !Mathf.Approximately(_target.InitialDelay, _selectedPreset.InitialDelay) ||
                   _target.Pivot != _selectedPreset.Pivot ||
                   ...
                   _target.AppearCurve != _selectedPreset.AppearCurve ||
                   ...
                   _target.LoopCurve != _selectedPreset.LoopCurve ||
                   ...
                   _target.DisappearCurve != _selectedPreset.DisappearCurve ||
                   _target.DisappearUsePositionCurve != _selectedPreset.DisappearUsePositionCurve ||
                   _target.EnableImpact != _selectedPreset.EnableImpact ||
                   _target.ImpactScale != _selectedPreset.ImpactScale ||
                   _target.ImpactPosition != _selectedPreset.ImpactPosition ||
                   _target.ImpactRotation != _selectedPreset.ImpactRotation ||
                   !Mathf.Approximately(_target.ImpactDuration, _selectedPreset.ImpactDuration) ||
                   _target.ImpactCurve != _selectedPreset.ImpactCurve ||
                   !Mathf.Approximately(_target.ImpactCharacterDelay, _selectedPreset.ImpactCharacterDelay);
        }
